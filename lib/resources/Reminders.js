...
Reminders.prototype.devareReminderByID = function devareReminderByID(id, callback) {
  var accessToken = this._uber.access_token;
  if (!accessToken) {
    return callback(new Error('Invalid access token'));
  }
  ...
  return this._uber.devare({ url: `${this.path}/${id}`, access_token: accessToken }, callback);
};
