...
Requests.prototype.create = function create(parameters, callback) {
    ...
    var localCallback = function localCallback(err, res) {
        if (!err) {
            ...
            if (meta.hasOwnProperty('surge_confirmation')) {
                return callback(err, res, meta.surge_confirmation);
            } else {
                return callback(err, res);
            }
        }
    };
    // check if the request being created is after surge acceptance
    if (parameters.hasOwnProperty('surge_confirmation_id')) {
