...
Uber.prototype.modifierMethodHelper = function modifierMethodHelper(options, callback, method) {
    ...
    if (options.params && options.params.auth_type && options.params.auth_type === 'server_token') {
        access_type = 'Token ' + this.defaults.server_token;
        delete options.params.auth_type;
    } else {
        // defaults to OAuth with access_token
        access_type = 'Bearer ' + ((options.access_token) ? options.access_token : this.access_token);
    }
...
Uber.prototype.get = function get(options, callback) {
    var url = this.defaults.base_url + ((options.version) ? options.version : 'v1') + '/' + options.url + '?';
