...
Uber.prototype.modifierMethodHelper = function(options, callback, method) {
  ...
  var params = {
    ...
    headers: {
  	...
  	body : ((options.params)? option.params : '')
  };
...
Uber.prototype.validateCoordinates = function validateCoordinates(lat, lon) {
  ...
  const locationReg = new RegExp("^[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?)\s*,\s*[-+]?(180(\.0+)?|((1[0-7]\d)|([1-9]?\d)‌​)(\.\d+)?)$");
