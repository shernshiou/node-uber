...
Uber.prototype.getCoordinatesForAddress = function getCoordinatesForAddress(address, callback) {
    geocoder.geocode(address, function(err, data) {
        if (err || data.results.length === 0) {
            return callback((err ? err : new Error('No coordinates found for: "' + address + '"')), {lat: '', lng: ''});
        }
...
Uber.prototype.replaceAddressWithCoordinates = function replaceAddressWithCoordinates(params, addressField, latField, lngField) {
    if (params.hasOwnProperty(addressField) && !params[latField] && !params[lngField]) {
        // get coordinates from address
        var coordinates = this.getCoordinatesForAddress(params[addressField]);
        if (coordinates) {
            delete params[addressField];
            params[latField] = coordinates.lat;
            params[lngField] = coordinates.lng;
        }
    }
